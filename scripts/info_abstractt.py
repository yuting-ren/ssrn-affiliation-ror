...
def info_abstract(output_info_abstract, begin_date, end_date, use_toydata=False, existing_dataset_path=None):
    ...
    df_result['abstract'] = df_result['abstract'].apply(clean_html_text)
    df_result['reference'] = df_result['reference'].apply(clean_html_text)
    df_result['affiliations'] = df_result['affiliations'].apply(clean_html_text)
      
    # 1) clean HTML tags
    df_result['abstract'] = df_result['abstract'].apply(clean_abstract)
    df_result['titre'] = df_result['titre'].apply(clean_abstract)
    df_result['reference'] = df_result['reference'].apply(clean_abstract)
    df_result['affiliations'] = df_result['affiliations'].apply(clean_abstract)
    
    

    # 2) remove line breaks (ASCII 10 / 13)
    df_result['abstract'] = df_result['abstract'].str.replace('\n', ' ').str.replace('\r', ' ')
    df_result['titre'] = df_result['titre'].str.replace('\n', ' ').str.replace('\r', ' ')
    df_result['reference'] = df_result['reference'].str.replace('\n', ' ').str.replace('\r', ' ')
    df_result['affiliations'] = df_result['affiliations'].str.replace('\n', ' ').str.replace('\r', ' ')
    ...
    df_result[["lang_abs", "lang_prob_abs"]] = (
        df_result["abstract"]
        .apply(lambda x: pd.Series(detect_lang(x)))
    )

    df_result["lang_match"] = df_result["lang"] == df_result["lang_abs"]
