...
# ---------- paths ----------
base_file_path = os.path.join(PROJECT_ROOT, "outputs", "db_info_abstract.csv")
base_output_path = os.path.join(PROJECT_ROOT, "outputs", "db_info_abstract_four_authors_ror.csv")
dict_path = os.path.join(PROJECT_ROOT, "outputs", "dictionary_new.csv")
...
nrows = int(row_limit) if row_limit else None

df = read_csv_with_fallback(file_path, sep=";", nrows=nrows)
dictionary = read_csv_with_fallback(dict_path, sep=";")
dictionary.columns = dictionary.columns.str.strip()
df.columns = df.columns.str.strip()
tqdm.pandas(desc="Four-author ROR")
...
match_4.loc[author4_needs_ror] = df.loc[author4_needs_ror, "author4_affi_raw"].progress_apply(ror_match)
