...
"""
Purpose:
- Keep only rows with exactly three authors from the main abstract dataset.
- Replace any known old affiliation names with simplified names from the dictionary,
  then recount "and" separators and split affiliations into three parts.
- Query the ROR API for author 1, author 2, and author 3 affiliation matches.
- Append new author1/author2/author3 old names to dictionary.csv without duplicating
  existing old names, and refresh the simplified name column.
...
Output:
- outputs/db_info_abstract_three_authors_ror.csv
...
base_output_path = "/Users/yutingren/Library/CloudStorage/Dropbox/Mac/Documents/AI/test_nov/abstract_analysis-main/outputs/db_info_abstract_three_authors_ror.csv"
...
tqdm.pandas(desc="Three-author ROR")
...
# ---------- keep exactly 3 authors ----------
df = df[(df["author_3_id"].notna()) & (df["author_4_id"].isna())].copy()
...
simplified_names = [
    str(name).strip()
    for name in dictionary["simplified name"].dropna().unique().tolist()
    if str(name).strip() != ""
]
...
df["affiliations_replaced"] = df["affiliations"].progress_apply(replace_affiliation)
...
# ---------- split rows with one comma and one "and" ----------
df["author1_affi_raw"] = None
df["author2_affi_raw"] = None
df["author3_affi_raw"] = None

mask_split = (df["and_count_new"] == 1) & (df["comma_count_new"] == 1)

def split_three_affiliations(text):
    if pd.isna(text):
        return (None, None, None)

    comma_parts = [part.strip() for part in str(text).split(",", maxsplit=1)]
    if len(comma_parts) != 2:
        return (None, None, None)

    left, right = comma_parts

    right_parts = re.split(r"\sand\s", right, maxsplit=1, flags=re.IGNORECASE)
    if len(right_parts) == 2:
        return (left, right_parts[0].strip(), right_parts[1].strip())

    return (None, None, None)

split_result = df.loc[mask_split, "affiliations_replaced"].apply(split_three_affiliations)
...
match_3.loc[author3_needs_ror] = df.loc[author3_needs_ror, "author3_affi_raw"].progress_apply(ror_match)
...
author_lookup = pd.concat([author1_lookup, author2_lookup, author3_lookup], ignore_index=True).drop_duplicates()
...
# Keep author metadata visible only for author_1 to author_4.
visible_cols = [
